...
produced substantial losses in some species....
...
Looking at the full
`r n_distinct(total_results$placement_id) * n_distinct(total_results$state_id)`
...
Make clear that a goal of all this complexity was to see if there were
any clear emergent properties (this thing matters way more than this
thing). We highlight some of those, but given the complexity we found we
present the complexity of interacting ways in which different things can
effect the conservation outcomes of MPAs

XX after looking through these should widdle it down to only things you
actually examine, annoying / no point to having a bunch of unexplained
minor variation in the mix relatede to things like ontogenetic shifts)
xx

xx the problem here. These things all interact in so many complex ways
that it's hard to look at either graphically or statistically, and maybe
for the best. i.e. the answer isn't "if habitat overlap is less than XX%
you might have a problem, otherwise you're fine", but rather that a
bunch of things can interact to cause potential problems in complex
ways. but that's hard to explain xx.
...
configurations, we fix habitat layers on the values predicted by
@bradley2021, and xx other things we fix xx.
