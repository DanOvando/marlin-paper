...
Specifically, the potential benefits of MPAs as a bycatch management are
...
fishery. Policy leakage is not a unique feature to MPAs, e.g. XX, but
the potential impacts of MPAs on target and bycatch species taking into
account the potential for these leakage effects has received relativelly
...
displacement and technical interactions. We use this model first to
...
an ideal case study in bycatch management through MPAs. The goal of this
paper is to provide a strategic evaluation of the sensitivity of bycatch
...
quality defines the quailty for adults or larvae of a given patch. The
...
objective function as the cell to remove from the network, and updateing
...
asign a price per KG of \$XX for tuna, intended to reflect a low value
...
for some overfishing when F/M is high enough, 70% for mariln, and 90%
for shark.

We ran `r nrow(state_experiments)` state experiments, varing the state
...
| Spatial catchability  |                       | Catchability varies in sapce                    |
...
experiments run, the results were highly variable. Across our
simulations, gains were much more common than losses, with losses of any
...
does not quite reach), with peak losses in our simulations occurring
...
forest using the `ranger` package (citation) in R (citation) and
...
from assumptions of homogenously distributed species with perfectly
...
processes in our model, by growth, natural mortality, and adult movment.
...
substantially amplified the predicted benefits of spatiao-temporal
...
allmetric fecundity when in fact the relationship was hyperallometric.
...
overfishing will likely find much higher value of optimization.

Value of information and fleet dynamics are two critical areas for
future study in this realm. Our study assumes perfect information in the
design of MPAs. We feel this is a useful benchmark for isolating the
...
fleet conditional on a fixed pool of fishing effort. As discussed
previously, the outcomes of MPAs depend critically on the reaction of
...
As demonstrated here there though there is a possibly endless list of
...
dependent as to defy easy classification of "important" or "not". We
feel these models are better viewed as a foundation for empirical
