...
Abstract: Designing effective spatial management strategies is
challenging because marine ecosystems are highly dynamic and opaque, and
extractive entities such as fishing fleets respond endogenously to
ecosystem changes in ways that depend upon ecological and policy
context. We present a tool -- marlin -- to efficiently simulate the
bio-economic dynamics of marine systems, designed to support both
management and research. We demonstrate `marlin’s` capabilities by
focusing on two case studies on the conservation and food production
impacts of marine protected areas (MPAs): a coastal coral reef and a
...
distributional outcomes of MPAs. In the pelagic case study, we
demonstrate how our model can be used to assess the climate resilience
of different MPA design strategies, as well as the climate sensitivity
of different fishing fleets. This paper demonstrates how intermediate
complexity simulation of coupled bio-economic dynamics can help
communities predict and potentially manage trade-offs between
conservation, fisheries yields, and distributional outcomes resulting
...
methods). However, they have tended to be either highly complex and
tactical models designed for use in a specific location, or to be
...
able to simulate one age-structured population distributed across a 100
patch system for 20 years in fractions of a second.
...
numbers, weight, and sexual maturity at age. We measure the size of the
population by th ratio of the *spawning stock biomass* (SSB) in a given
time step relative to *unfished spawning stock biomass* (SSB0),
SSB/SSB0. So, a SSB/SSB0 value of 1 means that the population is
...
longline fleet can be made much more likely to catch bigeye than
...
could in theory change in size and location. When MPAs are implemented,
...
at once. This means that the IFD for a single fleet would commonly be
modeled as a Nash-equilibrium based on the actions of each of the
individual fleet conditional on the actions of all other fleets. While
possible to implement, this would slow down our model runs to the point
of practically preventing large-scale evaluation of the spatial
management policies.
...
*advection* (follow currents), and *taxis* (active movement towards
...
matrix exponent of the sum of the diffusion $\boldsymbol{D}$ and taxis
$\boldsymbol{\tau}$ matrices scaled by the width of the time step
...
patch equals 100km^2^, then $\Delta_d = 100km$. This parameterization
...
The individual components of the diffusion matrix are filled based on an
adjacency matrix, which defines whether two patches are both adjacent
and water (as opposed to land or another physical barrier), and a
diffusion rate $D$ defined in units of length of the side of a patch per
unit of time. For example, if we are defining the time units as years
and the distance units as kilometers, for a tuna $D$ might be 1,000
$\frac{KM}{Year}$. The $\Delta_{t}$ and $\Delta_{d}$ parameters then
translate the movement rate $D$ to match the time step and patch size
used in a simulation. This "scale free" parameterization means that
appropriate value of $D$ can be identified for a species and then set,
regardless of the time step or patch size used in the simulation model
itself. These parameters are then used to calculate the value of a
particular entry the diffusion matrix (*d*).
...
The taxis matrix $\boldsymbol{\tau}$ is built off of the difference in
log habitat quality between each patch (the habitat gradient), the
adjacency matrix, and temporal and spatial resolution of the model
($\Delta_{t}$ and $\Delta_{p}$), as well as a scalar term $\lambda$.
$\lambda$ scales the magnitude of the habitat gradient between patches
relative to the diffusion parameter $D$. When the diffusion and taxis
matrices are estimated jointly from actual data in the manner of
@thorson2021a, $\lambda$ should be set to zero. However, when users are
manually creating both the diffusion and taxis matrices for simulation,
the relative strength of diffusion to taxis must be defined by the user.
Setting higher values of $\lambda$ allows users to simulate a scenario
where taxis is much more significant than diffusion.
...
pass their estimated parameters to `marlin` for simulation.
...
of density dependent recruitment:
...
4.  Post-dispersal density dependence: Larvae are distributed throughout
    ...
    the density of adult biomass at the destination patch.
...
code to view the precise details of each simulation. Targeted tactical
applications must carefully consider and document all decisions made
around model parameters.

#### MPA Design Strategies

Across all of our case studies we make use of two potential "rule of
...
can easily extend the analysis to compare outcomes between "contiguous"
(MPA is made up of one continuously connected block) and "mosaic" (MPAs
can be separated in space) MPA designs [@pons2022] by developing and
passing their own MPA design algorithms to the model.

#### Coral Reefs

In our coral reef example, we model the dynamics of four tropical
Pacific species: a grouper (*Epinephelus fuscoguttatus*), a shallow-reef
snapper (*Lutjanus malabaricus*), a deep-water snapper (*Pristipomoides
filamentosus*), and a reef shark (*Carcharhinus amblyrhynchos*). The
simulated groupers undergo a mass migration to a spawning aggregation
once per year, followed by the sharks. Shallow-reef snappers stay in
reefs closer to shore year-round, while deep-water snappers stay in the
deeper reefs (@fig-coral).
...
sharks, as any shark caught is retained. Fleet One is totally dependent
on fishing for their livelihood, meaning the local community takes
advantage of every possible opportunity to fish, and as such we model it
as a "constant effort" fishery.

Fleet Two primarily targets the deep-water snapper population. While
plate-sized snapper are still prized from this population, we model
Fleet Two's selectivity as logistic due to high levels of discard
...
which they sell their deep-water snapper. Accidental captures (bycatch)
do occur, which results in mortality. Fleet Two operates under
open-access dynamics, as fishing is not the primary means of
subsistence; short-term effort expands and contracts in response to
profitability of the primarily grouper-driven fishery.
...
($\Delta_t = 1/4$) and set the area of each patch to an arbitrary value
of 1 ($\Delta_p = 1$).
...
per year, over a 20 year time horizon (@fig-blue-ssb). We designed MPA
networks given the conditions in the starting year, and then held that
network constant over the years of the experiment, running one
simulation with and another without the climate-drive range shift. We
then compared the effects of this range shift on food production and
conservation outcomes from MPA networks designed based on the pre-range
shift world. We ran the simulation on a quarterly level
($\Delta_t = 1/4$) and set the area of each patch to an arbitrary value
of 1 ($\Delta_p = 1$).
...
For the coastal coral reef example the *Target fishing* MPA design
strategies was capable of producing a Pareto-improving, i.e. potentially
win-win, outcome where the total spawning biomass divided by unfished
spawning biomass across all species increase by up to 200% while
maintaining at least the same levels of fisheries yields as would be
caught in the absence of any MPAs. Conversely, achieving increases in
total fish biomass through the *Rate* strategy resulted in immediate
losses in total yields, and in losses to both yields and total biomass
in some cases (@fig-coral-results).
...
the simulated MPA networks accrued to fleet one, while catches by fleet
two were generally negatively impacted under both scenarios. For
example, the MPA sizes and designs that result in the largest increase
in yields for fleet one would produce a near total loss of fish catch
for fleet two.

The *Rate* MPA placement strategy produced lower total fishery gains but
minimized the negative impacts on fleet two. However, this strategy
produced much lower conservation gains than the *Target Fishing*
strategy, only benefiting deep snapper meaningfully, and in fact
resulting in slight losses to the nearshore snapper and grouper
populations for MPA sizes less than roughly 75% of the simulated space.
The *Target Fishing* strategy resulted in rapid conservation gains
across all of the simulated species.
...
strategies resulted in tradeoffs across at least one dimension. `marlin`
could be further used by a community such as the one simulated here to
explore alternative design strategies, or to assess options for
...
sensitive to climate impacts for the purse seine fishery than for the longline fishery. Under the
*status quo* species distribution, the *Target Fishing* MPA design
strategy produced increases in fisheries yields when the MPA covered
25-50% of the simulated area; that same MPA network resulted in the near
...
From a conservation perspective, spawning biomass of species such as
albacore and swordfish was relatively insensitive to the simulated range
shifts, whereas species such as bigeye tuna and silky shark showed
extreme variation in outcomes depending on the climate scenario. For
bigeye tuna, under the *status quo* conditions the *Target Fishing* MPA
design strategy resulted in rapid increases in the population, but that
same MPA network produced much smaller and slower gains under climate
change. However, for blue sharks this trend is inverted, with more rapid
...
applications. `marlin` allows for more realistic representations of
spatial social-ecological systems while maintaining processing speed.
...
realistic bio-economic dynamics on academic and applied problems. For
example, scientists could use `marlin` to consider the relative
importance of different biological variables in affecting policy
outcomes. Managers could use `marlin` to assess the potential of
proposed MPA networks to achieve stated objectives, such as maximizing
conservation gains for a threatened species while minimizing or removing
negative impacts to affected fishing fleets.
...
dynamics at play in the ocean. However, for all their limitations models
such as `marlin` can help users understand factors that drive the
...
All code and materials needed to reproduce this paper is available at
(https://github.com/DanOvando/marlin-paper)\[https://github.com/DanOvando/marlin-paper\].
