...
[@rcoreteam2021] to facilitate use, while the underlying population model is written in C++, integrated through the `Rcpp` package
[@eddelbuettel2018], for speed. Users on standard computers should be able to simulate one age-structured population distributed across a 100 patch system for 20 years in fractions of a second. 
...
We define a few commonly used terms here. *Yields* refers to the volume of catch from fishing activity. *Spawning stock biomass* refers to the total biomass of reproductively mature fish in the water, a function of numbers, weight, and sexual maturity at age. *Depletion* is a common term in the fisheries literature, and reflects here the *spawning stock biomass* in a given time step relative to *unfished spawning stock biomass*. So, a depletion value of 1 means that the population is unfished, a depletion value of 0 means the population is extinct.  
...
contrasting systems in which spatial management strategies such as MPAs are increasingly considered.

Below we provide a summary of the `marlin` model, as well as details of
the case studies. We note
...
These species can be caught by fishing fleets. A fishing fleet is defined
...
proportion to profit-per-unit-effort, conditional on management policies in places such as quotas and/or the presence of any spatial restrictions such as MPAs.
...
effort per fleet and the travel costs as a function of distance from a port.
...
(*R*), and costs (*C*) from fishing individual species (*s*) that it targets in a
given time step (*t*) and patch (*p*). The fleet then makes decisions
around fishing locations and intensity, subject to regulatory constraints,
...
amount of each species caught and its price ($\Pi$). The amount
...
We approximate the dome-shaped form as a normal distribution with
...
at once. This means that the IFD for a single fleet would commonly be modeled as a Nash-equilibrium based on the actions of each of the individual fleet
...
practically preventing large-scale evaluation of the spatial management policies. 
...
FishLife [internet connection required, @thorson2020]. All of the life history values can
alternatively be replaced with user-supplied values as needed and should be for more tactical applications; the default values simply ensure that reasonable and internally consistent
...
allows for movement to be broken down into three components of *advection* (follow currents), and *taxis* (active movement towards preferred habitat) and *diffusion* (essentially remaining variation in movement not explained by advection or taxis). For now, `marlin` focuses just on the
diffusion and taxis components of this model, assuming that advection is zero, though future extensions
...
matrix exponent of the sum of the diffusion $\boldsymbol{D}$ and taxis $\boldsymbol{\tau}$ matrices scaled by the width of the time step $\Delta_{t}$ and the length of the edge of each cell $\Delta_d$ that the model is running under. So for example, if we are running a model on a monthly timestep, and we define the units of time in years, then $\Delta_{t} = 1/12$. Similarly, if we are running a model where one cell equals 100km^2^, then $\Delta_d = 100km$. This parameterization allows users to set the effective area of the spatial domain through two avenues; the number of cells, which effectively scales the resolution of the model, and the area of each cell, which scales the spatial extent of the simulation. 

$$
\boldsymbol{M}_{t,s,a} = e^{\Delta_{t}(\boldsymbol{D}_{t,s,a} + \boldsymbol{\tau}_{t,s,a})} 
$$ {#eq-movement}

The individual components of the diffusion matrix are filled based on an adjacency matrix, which defines whether two cells are both adjacent and water (as opposed to land or another physical barrier), and a diffusion rate $D$ defined in units of length of the side of a cell per unit of time. For example, if we are defining the time units as years and the distance units as kilometers, for a tuna $D$ might be 1,000 $\frac{KM}{Year}$. The $\Delta_{t}$ and $\Delta_{p}$ parameters then translate the movement rate $D$ to match the time step and cell size used in a simulation. This "scale free" parameterization means that appropriate value of $D$ can be identified for a species and then set, regardless of the time step or cell size used in the simulation model itself. 
...
The taxis matrix $\boldsymbol{\tau}$ is built off of the difference in log habitat quality between each patch, the adjacency matrix, and temporal and spatial resolution of the model ($\Delta_{t}$ and $\Delta_{p}$), as well as a scalar term $\lambda$. $\lambda$ scales the magnitude of the habitat
gradient between patches relative to the diffusion
...
the diffusion and taxis matrices for simulation, the relative strength of diffusion to
...
\tau_{p1,p2,t,s,a} = \begin{cases}
      = \frac{\Delta_{t}}{\Delta_{d}}(1 + \lambda d_{s,f})e^{h(p2,t,s,a) - h(p1,t,s,a)} & \text{if p2 and p1 are adjacent}\\
...
setting habitat in a patch to zero; an animal will not preferentially move
towards a patch with zero habitat all else being equal, but can move
over a zero habitat patch towards another better patch. They most move around a physical barrier. 
...
in a heavily fished coastal coral reef ecosystem. In the second, we demonstrate
...
1.  *Rate*: MPAs are placed based on the pre-MPA
...
2.  *Target Fishing*: MPAs locations are prioritized
    proportional to fishery catches. Patches with high total
...
through the use of a modeling framework, and depending on the
validity of the model, this process is likely to produce better
...
can easily extend the analysis to compare outcomes between
"contiguous" (MPA is made up of one continuously connected block) and "mosaic" (MPAs can be separated in space) MPA designs [@pons2022].
...
shore year-round, while deep-water snappers stay in the deeper reefs
(@fig-coral).
...
fleet, where the local community takes advantage of every possible opportunity to
fish, and as such we model it as a "constant effort"
fishery.
...
which they sell their deep-water snapper. Accidental captures (bycatch) do
occur, which results in mortality. This fleet operates under open-access
dynamics, as fishing is not the primary means of subsistence;
...
dictated by the data). We ran the simulation on a quarterly level ($\Delta_t = 1/4$) and set the area of each cell to an arbitrary value of 1 ($\Delta_p = 1$).
...
per year, over a 20 year time horizon (@fig-blue-ssb).
...
the pre-range shift world. We ran the simulation on a quarterly level ($\Delta_t = 1/4$) and set the area of each cell to an arbitrary value of 1 ($\Delta_p = 1$).
...
For the coastal coral reef example the *Target fishing* MPA design strategies was capable of producing a Pareto-improving, i.e. potentially win-win, outcome where the total
...
Conversely, achieving increases in total fish biomass through the *Rate* strategy resulted in immediate losses in total yields, and in losses to both yields and total biomass in some cases
(@fig-coral-results).

These macro-level changes in total yield and conservation outcomes mask
some internal tradeoffs. Nearly all of the food production benefits from the simulated MPA networks accrued to fleet one, while catches by fleet two were generally
negatively impacted under both scenarios. For example, the MPA sizes and designs that result in the largest increase in yields for fleet one would produce a near total
loss of fish catch for fleet two.

The *Rate* MPA placement strategy produced
lower total fishery gains but minimized the negative impacts on fleet
two. However, this strategy produced much lower conservation gains
than the *Target Fishing* strategy, only benefiting
deep snapper meaningfully, and in fact resulting in slight losses to the
nearshore snapper and grouper populations for MPA sizes less than roughly 75% of the
simulated space. The *Target Fishing* strategy
...
biomass and yields in the coastal coral reef example, but all the
tested strategies resulted in tradeoffs across at least one dimension. `marlin` could be further used
by a community such as the one simulated here to explore alternative design
strategies, or to assess options for distributing fishery benefits
across communities that may experience very different food provision outcomes from the
same MPA network.

```{r}
#| label: fig-coral-results
#| fig-cap: "Percent change in yield per fleet (A) and depletion (spawning biomass divided by unfished spawning biomass) per species (B) as a function of MPA size and MPA design strategy for the coastal coral reef case study. Percent change in total yield across both fleets (y-axis) and total depletion across all species (x-axis) and placement strategies (line color) (C). Text labels in panel C indicate the percent of area in an MPA at selected points along curves."
#| fig-height: 4
#| fig-width: 6
...
*status quo* conditions the *Target Fishing* MPA design strategy
resulted in rapid increases in the population, but that same MPA network produced much smaller and slower gains under climate change.
...
#| fig-cap: "Change in yield (A) and depletion (spawning biomass divided by unfished spawning biomass) (B) as a function of MPA size and design strategy by fishing fleet and species. Blue lines indicate impacts of MPAs under status qup habitat, red impacts under climate-driven range shift."
#| fig-height: 6
#| fig-width: 6
...
blue_critter_bio <- blue_water_results %>%
  ...
  scale_y_continuous(name = "Depletion",limits = c(0, NA)) +
...
Communities must often make decisions on how to manage these systems based on
...
tools for spatial-temporal modeling available in the literature, each of which provide useful functionality for specific applications. However, we highlight here the specific gaps that we feel `marlin` fills
...
extremely time consuming to design and run. `marlin` allows some of the realism of these end-to-end models while being simpler and faster to parameterize and run.  
...
applications. `marlin` allows for more realistic representations of spatial social-ecological systems while maintaining processing speed. 
...
the same MPA to benefit some fleets and species while harming others when fishing
fleets affected by an MPA do not share the same objectives and species are not uniformly distributed. Our pelagic
