...
#| fig-cap: "Percent change in yield per fleet (A) and depletion per species (B) as a function of MPA size and MPA design strategy for the coastal coral reef case study. Percent change in total yield across both fleets (y-axis) and total depletion across all species (x-axis) by MPA size (point size) and placement strategy (point color) (C)." 
#| fig-height: 4
#| fig-width: 6
...
coral_frontier <- coral_results %>% 
  ...
  ggplot(aes(delta_biodiv, delta_yield, fill = placement_strategy, size = prop_mpa)) + 
  ...
  scale_size(name = "MPA Size", labels = scales::label_percent(accuracy = 1), breaks = seq(0.25,1, by = 0.25)) +
  guides(fill = guide_legend(order = 1, override.aes = list(size = 4)),
         size = guide_legend(order = 2, ncol = 2, byrow = TRUE)) + 
  labs(tag = "C")  + 
  theme(
    legend.position = c(0.725, .85),
