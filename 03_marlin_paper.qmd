...
sources, the amount of biomass killed through fishing being assigned to
yield *Y*.
...
Selectivity $\alpha$ is modeled through either a logistic form or a
dome-shaped form. The logistic form is based on the lengths *l* at which
50% of individuals are selected by the fishing gear, $l^{sel}$, and
$\delta$ which is the difference between the length at 50% selectivity
and the length at 95% selectivity.
...
$$ Lastly *z* is total mortality, the sum of fishing mortality and
natural mortality (*m*).
...
Costs *C* are calculated as a function of $\gamma_f$, that the scales
the cost per unit effort for fleet *f*,$\beta$ allows for the cost of
effort to scale non-linearly, and $\eta$ is an additional cost per unit
effort of fishing in each patch *p*, allowing for the model to account
for travel costs for different patches. Travel costs are calculated
based on the euclidean distance of each patch to the nearest port of a
given fleet, users can specify any number from zero to the number of
patches of port locations. When no ports are specified, travel costs are
zero.
...
and approximates the percentage amount effort changes in response to a
one unit changes in the log revenue to cost ratio.

$$
E_{t+1,f} = E_{t,f} \times e^{\theta_f log(R_{t,f} / C_{t,f})} 
$$ {#eq-oa}

When MPAs are implemented, effort that used to operate inside the MPAs
can either leave the fishery, or be redistributed outside the the MPA
(the default behavior). The model can also accept total catch quotas for
each species in each time step. When quotas are activated, if the total
catch across all fleets for a given species under the model's effort
dynamics would exceed the allowable quota for that species, the total
effort for each fleet is decrease proportional to its contribution to
the total catch until the quota is satisfied. As an alternative to catch
quotas, users can input an effort cap per fleet, which prevents effort
from exceeding a given amount under open-access dynamics, though effort
may be reduced below this cap if required by the profit equation.
`marlin` also allows users to specify closed fishing seasons for one or
more species in the system. During closed seasons, no effort is
allocated to patches that would result in capture of the covered
species, potentially resulting in zero effort if the species cannot be
avoided. Under these conditions, the amount of effort in the next open
...
on the values reported in FishLife [@thorson2020]. All of these values
can be replaced with user-supplied values as needed though, the defaults
...
spawning-stock-biomass. $l_a$ is the mean length at at age, $l_sel$ is
...
the length at which on average 50% of individuals are sexual maturity,
and $\delta_{mat}$ is the units of length beyond $l_{mat}$ at which on
...
though the exact level of stock status desired by societal objectives
may vary widely. MS based reference points present a problem for a
multi-fleet and spatial-temporal model such as `marlin`. MSY and the
fishing mortality rate that would produce MSY, $F_{MSY}$ are a function
of fishery selectivity. Fishery selectivity in this model can vary by
fleet, and species can be distributed unevenly in both space and time.
This means that the net fishing selectivity on a species can vary
...
($SSB_{MSY}$) which is more conserved across selectivity patterns as it
is primarily a function of the mortality and flow of recruits relative
to the maturity ogive. There are many different strategies for
estimating reference points conditional on selectivity and regimes, see
citations xx. We leave it to users to find relevant reference point as
required by their methods on their own, using the sort of MSE examples
...
Each of the use cases contains too many parameters and options to be
...
code (provide clear instructions for which files) to view the precise
details of each simulation. xx something like this to clarify why we
don't have pages of tables with all the parameters and options).
Targeted tactical applications though should be sure to consider and
document all decisions made around model parameters.
...
#| fig-cap: "Frontier in conservation and yield outcomes under different MPA design strategies (A) for coral reef case study. Change in yield for each fleet (B) and depltion per species (C) as a function of MPA size and MPA design strategy for the coral reef case study."
#| fig-height: 4
#| fig-width: 6
...
assessment. In addition, the process-based movement model used in
`marlin` can directly use empirical estimates of movement dynamics
derived in the manner of @thorson2021a, providing a link between
empirical and simulation based approaches to marine resource management.
Future iterations of the model may include life history modules for a
broader range of species types such as marine mammals, sea birds, and
sea turtles.
