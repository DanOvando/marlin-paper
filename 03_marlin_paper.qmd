...
amount caught of each species and its price ($\Pi$). The amount
...
allows for movement to be broken down into three components of *advection* (follow currents), and *taxis* (active movement towards preferred habitat) and *diffusion* (essentially remaining variation in movement not explained by diffusion or taxis). For now, `marlin` focuses just on the
...
2.  Target_fishing (*target_fishing*): MPAs locations are prioritized
    proportional to fishery catches. So, patches with high total
...
dictated by the data). We ran the simulation on a quarterly level, meaning that $\Delta_t = 1/ 4$ and set the area of each cell to an arbitrary value of 1 (meaning $\Delta_p = 1$ ).
...
the pre-range shift world. We ran the simulation on a quarterly level, meaning that $\Delta_t = 1/4$ and set the area of each cell to an arbitrary value of 1 (meaning $\Delta_p = 1$ ).
...
this MPA network accrued to fleet one, while catches by fleet two were generally
