...
Manuscript Title: Simulating Benefits, Costs, and Tradeoffs of Management Policies in Marine Social-Ecological Systems

Author Names: Daniel Ovando^1,2^, Darcy Bradley^3,4,5^, Echelle Burns^3,4,5^,Lennon Thomas^3,4,5^, James Thorson^6^

Affiliations:
^1^ School of Aquatic and Fishery Sciences, University of Washington,1122 NE Boat St Box 355020,Seattle, WA, USA
^2^ Current Address: Inter-American Tropical Tuna Commission,8901 La Jolla Shores Drive, CA, USA
...
^5^ Environmental Markets Lab, University of California, Santa Barbara, CA, USA
^6^ Habitat and Ecological Processes Research Program, Alaska Fisheries Science Center, NOAA, NMFS, NOAA, Seattle, WA, USA

Corresponding Author: Daniel Ovando <dovando@iattc.org>

Open Research Statement: All code and materials needed to reproduce this paper is available at [https://github.com/DanOvando/marlin-paper](https://github.com/DanOvando/marlin-paper).

Key Words: 
Bio-economic modeling, spatio-temporal modeling, marine protected areas, fisheries management, marine spatial planning, conservation planning
...
@thorson2021a for a detailed description). Under this framework, movement of indivuduals
...
The individual components (*M*) of the movement matrix ($\boldsymbol{M}$) are filled based
...
The movement of individuals across patches is then calculated by matrix multiplication of the pre-movement vector of the number of individuals ($\boldsymbol{n}$) of species *s* at age *a* in time step *t* across all patches *p* times the matrix exponential of the movement matrix $\boldsymbol{M}$

$$
\boldsymbol{n}_{t+1,s,a} = \boldsymbol{n}_{t,s,a}e^{\boldsymbol{M}_{t,s,a}}
$$ {#eq-movement}
...
us to simulate preferences of species in space and time efficiently.
Lastly, this form has the advantage that the parameters are directly
estimable from real data. So, if provided with for example spatial
abundance data and a tagging study, users can estimate the diffusion and
taxis movement parameters in the same manner as @thorson2021a, and then
pass their estimated parameters to `marlin` for simulation (so long as
the estimating method uses the same functional form as the movement
model in `marlin`).
...
This project was funded by the Waitt Foundation. We thank Dr. Alexa Fredston for helpful revisions of this manuscript. 

## Author Contribitions

Methods and code written by DO and JT. Results produced by DO. Data collected by EB and LT. All authors contributed to the conceptualization and writing of the manuscript. 
