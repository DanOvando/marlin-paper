...
selectivity for individual species (each of which we denote as a *metier*). For
example, both a longline and purse seine fleet may capture bigeye (*Thunnus
obesus*) and skipjack tunas, but the longline fleet can be made much more likely
to catch larger bigeye than skipjack, and *vice versa*. Each fishing fleet
distributes its fishing effort in space according to a specified spatial
allocation rule (see *Spatial Allocation of Effort* ), for example in proportion
to profit-per-unit-effort, conditional on management policies in places such as
...
set the area of each spatch to be 5km^2^ ($\Delta_p = \sqrt{5}$), using 400
...
For the coastal coral reef example both the *Spawning Ground* and *Target
fishing* MPA design strategies were capable of producing a Pareto-optimal or
improving, i.e. potentially win-win, outcome where change in total spawning
biomass across all species and yield across all fleets was greater than or equal
to zero for some MPA size. However, producing a total increase in SSB/SSB0
greater than 50% came at a net cost to fisheries yield across both design
strategies. The *Spawning Ground* strategy produced more immediate trade-offs
between food security and conservation (@fig-coral-results).

These macro-level changes in total yield and conservation are the result of
interesting internal dynamics that `marlin` allows users to explore through
output tables at the level of individual species and fleets. The *Spawning
Ground* strategy increased yield to Fleet One when MPAs were relatively small,
but was able to maintain yields near *status quo* levels up to a very large MPA
size to the dependence of Fleet One on only a relatively small portion of the
total simulation area. However, *Spawning Ground* MPAs were unable to provide
...
MPAs were capable of producing positive outcomes in terms of total biomass and
yields in the coastal coral reef example, but all the tested strategies resulted
in trade-offs across at least one dimension. `marlin` could be further used by a
community such as the one simulated here to explore alternative design
strategies, or to assess options for distributing fishery benefits across
communities that may experience very different food provision outcomes from the
same MPA network.
...
*status quo* conditions (@fig-blue-results). In terms of fisheries yield, the
...
xx add more details on case study here xx

-   Coral

    -   targeting spawning grounds for MPA provides better / more equitable
        yield outcomes, but at the expense of much slower conservation gains,
        and even some negative tradeoffs for species like deep snapper.
        Targeting fishing provides better gains to fleet two (explain why), and
        better conservation outcomes, but squarely at the expense of fleet one

-   Blue Water

    -   target-fishing design being much more robust to range shifts than rate
        design, why?

    -   High concentration of the purseseine fleet means much more sensitive to
        placement "errors"

    -   Basic life history poses challenge to recovery of severely depleted
        species e.g. OWT
...
and food security were relatively insensitive to the form of fishing selectivity
(Fig.S1). This does not mean that knowing the "correct" form of selectivity may
not be extremely important in providing accurate assessment results
[@waterhouse2014], but rather that holding other parameters constant, in this
case misspecifying the selectivity curve is projected to have little impact on
the simulated outcomes of MPAs.
...
dynamics through agent-based approached [@burgess2020]. Particularly for more
...
In sum, `marlin` complements the existing spatial marine modeling literature by
allowing scientists, decision makers, and other stakeholders to efficiently
examine the impacts of realistic bio-economic dynamics on academic and applied
problems. For example, scientists could use `marlin` to consider the relative
importance of different biological variables in affecting policy outcomes.
Managers could use `marlin` to assess the potential of proposed MPA networks to
achieve stated objectives, such as maximizing conservation gains for a
threatened species while minimizing or removing negative impacts to affected
fishing fleets.
...
policy interactions, and spatial stock assessment. In addition, the
process-based movement model used in `marlin` can directly use empirical
estimates of movement dynamics derived in the manner of @thorson2021a, providing
a link between empirical and simulation based approaches to marine resource
management. The model currently focuses on diffusion and taxis, which allows for
general representation of animals following physical or oceanographic features,
but research on the incorporation and importance of advective forces would be of
value going forward. Future iterations of the model may include life history
modules for a broader range of species types such as marine mammals, sea birds,
and sea turtles.
...
This project was funded by the Waitt Foundation. We thank Dr. Alexa Fredston for
helpful revisions of this manuscript.
