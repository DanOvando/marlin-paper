...
params:
  run_name: ["v0.3"]
abstract: "Communities around the world are increasingly looking to spatial strategies for managing marine ecosystems. However, the design of spatial management strategies faces numerous challenges. Marine ecosystems are highly dynamic and opaque, and extractive entities such as fishing fleets respond endogenously to changes in marine ecosystems in ways that depend upon ecological and policy context. We present a framework for efficient simulation of the bio-economic dynamics of marine systems, designed to support both management and research. We demonstrate the model's capabilities by focusing on two case studies on the conservation and food security impacts of marine protected areas (MPAs). We highlight how coupled bio-economic dynamics can result in complex trade-offs between conservation, fisheries yields, and distributional outcomes. "
header-includes:
...
spatial management, across 30% of the world's oceans by the year 2030.
While the objectives of individual spatial management strategies vary
widely, these types of policies are often supported based on their
ability to provide protection, along with potential broader benefits, to
swaths of marine ecosystems with one management action, with potentially
fewer costs and complications than for example assessing, setting, and
enforcing catch quotas for individual species.
...
ecosystems in highly complex ways. These dynamics pose challenges even
...
a protected area network given a wide array of life history, species
distributions, exploitation levels, fleet dynamics, and policy-dependent
behavior [@reimer2017], all of which may change over time, particularly
given the impacts of climate change.

These challenges are amplified by the fact that our knowledge regarding
the succesful design and implementation of spatial management strategies
such as MPAs is largely based on theoretical studies; the size
[@ovando2021a] and time required [@nickols2019] to see substantial
effects from many MPAs means that we have relatively little empirical
evidence as to their large-scale effects across a range of
...
framework called [`marlin`](https://danovando.github.io/marlin/) that
allows for efficient simulation of spatio-temporal biological and
economic dynamics. `marlin` allows users to simulate the impacts
fisheries management policies across a range of species targeted by
various fleets across heterogeneous and dynamic seascapes. This model
can help users seek Parteo-optimal solutions that produce win-wins or
minimize tradeoffs across multiple objectives, such as food security and
conservation [@lester2013].
...
costs, and tradeoffs between conservation and food security outcomes of
MPAs. More broadly, this paper both demonstrates the functionality of
...
As a demonstration of the use of this modeling framework, we explore two
general situations

1.  Trade-offs and distributional outcomes for food security and
    ecosystem biomass in a coral-reef fishery
...
foundations of our framework for efficiently simulating bio-economic
...
dependence implemented in the model, with potentially auto-correlated
deviations from deterministic reproductive dynamics.
...
capture bigeye (*Thunnus obesus*) and skipjack (*Katswamus pelamus*)
tunas, but the longline fleet can be made much more likely to catch
bigeye than skipjack, and *vice versa*. Each fishing fleet distributes
its fishing effort in space according to a specified spatial allocation
rule, for example in proportion to profit-per-unit-effort, conditional
on the presence of any spatial restrictions such as MPAs.
...
redistributed outside the the MPA (the default behavior).
...
for a coral reef or pelagic system.
...
based on th total profitability across all species in space and time.
...
alone, and so we include those scenarios here to allow users to evalaute
...
per-unit-effort strategy may be more realistic, in which fishers
distribute themselves based on the expected catch of their individual
efforts [@hilborn1987]. Conversely, a system defined by a sole owner
seeking to maximize total profits might be better represented by a fleet
model based on total profits
...
FishLife [@thorson2020]. All of the life history values can be replaced
with user-supplied values as needed though, the defaults simply ensure
that reasonable and internally consistent values for the species in
question are supplied as a starting point.
...
this way then, `marlin` allows users to run anything from a simple
...
life stage *a* of species *s* is defined by a movement matrix
...
diffusion rate, see @thorson2021a.
...
may vary widely. MSY based reference points present a problem for a
...
This means that the net effective fishing selectivity on a species can
vary depending on the dynamics at a given moment, making the definition
of an equilibrium concept such as MSY challenging.
...
citations xx. We leave it to users to define and find relevant reference
point as required by their specific needs.
...
climate resilience of alternative MPA design strategies.
...
precise details of each simulation. Targeted tactical applications
though should be sure to consider and document all decisions made around
model parameters.
...
broadly usable design strategies. We allow all MPAs to be designed in a
"mosaic" fashion, but users can easily extend the analysis to for
example compare outcomes between "contiguous" and "mosaic" MPA designs
[@pons2022].
...
which they sell their deep-water snapper, though they do accidentally
catch and kill them on occasion. This fleet operates under open-access
dynamics, as fishing is not the primary means of subsistence and so
short-term effort expands and contracts in respond to profitability of
...
We simulated the outcomes for both food security and conservation for
each of the species and both of the fleets as a function of both MPA
...
selectivity values pulled from the literature where available. Both
fleets operate under open-access dynamics with an effort cap. The effort
cap was set at the level of effort that resulted in the desired levels
of depletion for each species under open-access dynamics (@fig-blue),
intended to simulate a scenario where managers step in to prevent
further expansion of fishing effort in a fully developed fishery. For
forward-simulation, open-access dynamics can result in effort decreasing
in response to profitability, but cannot result in effort beyond the
effort cap set for each fleet.
...
shift. We then compared effect of this range shift on food security and
...
strategies were capable of producing a Pareto-improving, i.e.
potentially win-win, outcome where the total spawning biomass divided by
unfished spawning biomass across all species could be increased by up to
200% while maintaining at least the same levels of fisheries yields as
would be caught in the absence of any MPAs. Conversely, achieving
increases in total fish biomass through the rate or avoid_fishing
strategies resulted in immediate losses in total yields, and in losses
to both yields and biomass in some cases (@fig-coral-results).
...
Both the rate and avoid_fishing MPA placement strategies produced lower
total fishery gains but minimized the negative impacts on fleet two.
However, these strategies produced much lower conservation gains than
the area and target_fishing strategies, only benefiting deep_snapper
meaningfully, and in fact resulting in slight losses to the nearshore
snapper population for MPA sizes less than roughly 75%. Both the area
and target_fishing strategies resulted in rapid conservation gains
...
#| fig-cap: "Frontier in conservation and yield outcomes under different MPA design strategies (A) for coral reef case study. Change in yield for each fleet (B) and depletion per species (C) as a function of MPA size and MPA design strategy for the coral reef case study."
#| fig-height: 4
#| fig-width: 6
...
management policies informed by better understanding of sensitivities to
key uncertainties.

<!-- `marlin` is to our knowledge the first bio-economic modeling tool that -->

<!-- allows scientists and decision makers to efficiently simulate the -->

<!-- outcomes of management strategies such as MPAs across multiple fleets -->

<!-- and species while accounting for layered regulations, economic -->

<!-- incentives, age structure, and realistic movement dynamics across a -->

<!-- range of potential social-ecological conditions. xx this sentence annoys -->

<!-- me but feels like we need to make something like it explicit to convince -->

<!-- reviewers maybe less familiar with the modeling literature xx. -->
...
such as virtualspecies [@leroy2016] or STEPS [@visintin2020] can help
...
demonstrates the value in our method. Beyond the current examples, we
envision `marlin` being applicable to research on dynamic ocean
management, range shifts, management strategy evaluation, policy
interactions, and spatial stock assessment. In addition, the
process-based movement model used in `marlin` can directly use empirical
estimates of movement dynamics derived in the manner of @thorson2021a,
providing a link between empirical and simulation based approaches to
marine resource management. Future iterations of the model may include
life history modules for a broader range of species types such as marine
